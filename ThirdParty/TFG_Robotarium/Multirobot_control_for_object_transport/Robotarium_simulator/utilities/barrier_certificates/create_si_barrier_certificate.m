...
%       size of the robots.
    ...
    function [ dx ] = barrier_certificate(dxi, x)
        ...
        N = size(dxi, 2);   % N = nº columnas de dxi (cantidad de puntos en los que descompone el trayecto ¿?)
        ...
        x = x(1:2, :);      % Filas 1 y 2 = posición x,y ¿?
        ...
        for i = 1:(N-1)
            for j = (i+1):N
                % h es un vector x,y, donde resta el cuadrado de la
                % diferencia de las posiciones actuales de 2 robots y el
                % cuadrado de la distancia de seguridad
        ...
        vhat = reshape(dxi,2*N,1);  % Reestructura la matriz dxi en un solo vector de 2N filas y 1 columna
        H = 2*eye(2*N);
        f = -2*vhat;
        ...
        dx = reshape(vnew, 2, N);   % Reestructura la matriz vnew en una matriz de 2 filas (velocidades en x,y) y N columnas (N robots)
    end
end
