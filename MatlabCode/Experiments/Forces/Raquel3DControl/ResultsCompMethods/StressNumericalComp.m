...
[errors_cbf.strain, errors_cbf.stress, errors_cbf.vm_stress] = GetFEMErrors(vm_stress_cbf,stress_tensor_cbf,strain_tensor_cbf,u_hat_cbf,Le_cbf,Ce_cbf,omesh,true, "Standard CBF");
...
% Equivalence evaluation
[errors_cbf.pred_strain, errors_cbf.pred_stress, errors_cbf.pred_vm_stress] = GetFEMErrors(pred_vm_stress_cbf,pred_stress_tensor_cbf,pred_strain_tensor_cbf,pred_u_hat_cbf,Le_cbf,Ce_cbf,omesh,true,"Predicted CBF");

%% Nodal Displacement
disp(" ");
disp("########### Nodal displacements ###########");
next_u_hat_cbf = reshape(u_hats_cbf(:,:,iter+1),[12, 1, size(u_hats_cbf(:,:,iter+1),2)]); % (3*4 x 1 x N_tet)
errors_cbf.u_hat_prediction = abs(next_u_hat_cbf - pred_u_hat_cbf);
CompareValues(errors_cbf.u_hat_prediction,true,"U hat estimation", "CBF");
CompareValues(abs(next_u_hat_cbf - u_hat_cbf),true,"U hat Increment", "CBF");
...
% Equivalence evaluation
[errors_3D.strain, errors_3D.stress, errors_3D.vm_stress] = GetFEMErrors(vm_stress_3D,stress_tensor_3D,strain_tensor_3D,u_hat_3D,Le_3D,Ce_3D,omesh,true, "Standard 3D");
...
% Equivalence evaluation
[errors_3D.pred_strain, errors_3D.pred_stress, errors_3D.pred_vm_stress] = GetFEMErrors(pred_vm_stress_3D,pred_stress_tensor_3D,pred_strain_tensor_3D,pred_u_hat_3D,Le_3D,Ce_3D,omesh,true,"Predicted 3D");

%% Nodal Displacement
disp(" ");
disp("########### Nodal displacements ###########");
next_u_hat_3D = reshape(u_hats_3D(:,:,iter+1),[12, 1, size(u_hats_3D(:,:,iter+1),2)]); % (3*4 x 1 x N_tet)
errors_3D.u_hat_prediction = abs(next_u_hat_3D - pred_u_hat_3D);
CompareValues(errors_3D.u_hat_prediction,true,"U hat estimation", "3D");
CompareValues(abs(next_u_hat_3D - u_hat_3D),true,"U hat Increment", "3D");
